    ...
    <>
      <div className='w-full gap-2 flex-wrap bg-white shadow-drop rounded-2xl flex items-center p-4 border-b border-gray-200 text-gray-600'>
        <Dropdown
          ...
          defaultValue={`${selectMPOptions[1].label}- ${selectMPOptions[1].store}`}
        ...
        <button
          onClick={handleApply}
          className='shadow-drop items-center justify-center font-bold  rounded-lg border border-gray-600 h-[37px] uppercase ml-2 px-4 py-1 text-sm text-customGray  hover:bg-customOrange hover:text-white'
        >
          Edit
        </button>
        <button
          onClick={handleApply}
          className='shadow-drop flex items-center justify-center w-[105px] font-bold rounded-lg border border-gray-600 text-customGray h-[37px] uppercase ml-2 px-4 py-1 text-sm hover:bg-customOrange hover:text-white'
        >
          <img src={refreshIcon} alt='Refresh Icon' className='mr-1' />
          Refresh
        </button>
        <span className=' underline text-sm  '> What's this?</span>
        ...
        <div className='flex flex-wrap w-full gap-2 text-sm mt-2 ml-2'>
          ...
          <div className='underline'>Click here</div> to change it
        </div>
      </div>
    </>
  );
