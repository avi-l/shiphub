...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav className='flex sticky items-center p-4 gap-2 top-0 w-full bg-white'>
        <img src='/shiphub.svg' alt='Shiphub Logo' />
        <Dropdown
          defaultValue={<div className='font-bold'>Hi, Cookies Kids!</div>}
          isOpen={isDropOpen === 'Hi, Cookies Kids!'}
          toggleDropdown={() => toggleDropdown('Hi, Cookies Kids!')}
          items={USER_OPTIONS}
          icon={downArrowSolid}
          labelClassName='relative bg-none'
          buttonClassName='flex font-light items-center rounded-md  text-gray-600 h-[38px]'
          iconBoxClassName='p-1 m-1  '
          dropdownClassName=' z-50 left-0 mt-10 w-48 bg-white border border-gray-300 rounded-md shadow-lg text-gray-600'
        ...
        <Dropdown
          ...
          icon={downArrow}
          buttonClassName='flex font-light  items-center px-4 py-2 rounded-md bg-white border border-gray-300 text-gray-600 h-[38px] w-[292px] relative shadow-input'
          iconBoxClassName='absolute top-0 right-0 bottom-0 rounded-r-md bg-customOrange flex items-center justify-center w-[48px]'
          dropdownClassName='absolute right-0 mt-10     w-48 bg-white border border-gray-300 rounded-md shadow-lg text-gray-600'
          itemClassName='block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100'
        />
        <Dropdown
